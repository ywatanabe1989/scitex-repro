...
This module also wires module-import-time coverage so that
child Python interpreters (subprocess.run, jupyter nbconvert
--execute, pytest-xdist workers) write their coverage shards
into the repo root and get combined at session end.

`os.environ.setdefault` would be a no-op here because
pytest-cov has already set COVERAGE_FILE to a tmp dir by the
time conftest is loaded — so we *force-set* both vars and
...
# Pin coverage's data file at the repo root and point
# process_startup at our pyproject so child interpreters
...
def _ensure_subprocess_coverage_shim() -> None:
    ...
    pth = purelib / "_scitex_repro_subprocess_coverage.pth"
    shim = (
        "import os, coverage\n"
